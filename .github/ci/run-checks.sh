...
# 1. Parâmetro de negócio conforme TRUTH-002 (margem mínima 12% — Issue #2)
if ! grep -q "const MARGEM_MINIMA = 0.12;" calculadora.html; then
  echo "FALHA: MARGEM_MINIMA difere de 0.12 (TRUTH-002)."
...
if [ "$fail" -eq 0 ]; then echo "Checks OK: parametros e golden cases conformes."; fi
exit "$fail"
